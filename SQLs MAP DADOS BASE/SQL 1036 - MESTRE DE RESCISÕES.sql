...
/*SELECT * FROM CONTRATOS_OK
WHERE COD_CONTRATO = 398418*/
...
SELECT DISTINCT ORG.COD_NIVEL2 AS "codigo_empresa",
 ...
 WHERE A.COD_CONTRATO IN (SELECT COD_CONTRATO FROM CONTRATOS_OK)
   ...
   AND A.COD_CONTRATO NOT IN (175668, 191931, 233552, 234575, 262129, 262196, 263290, 263524, 248339,
                              263800, 266426, 267899, 268534, 270644, 271837, 278700, 282022, 287512)
   AND A.COD_CONTRATO = 390418
...
WHERE DATA_RESCISAO < '23/04/2026'
AND COD_CONTRATO = 390418
AND COD_CONTRATO NOT IN (175668, 191931, 233552, 234575, 262129, 262196, 263290, 263524, 
263800, 266426, 267899, 268534, 270644, 271837, 278700, 282022, 287512)

SELECT * FROM RHFP0310
WHERE COD_CONTRATO = 248339

SELECT * FROM RHFP0351
where COD_CONTRATO = 396381
